...
describe("本地管理员首次登录初始化", () => {
  it("只能在本地空数据库创建一次，且不会被环境变量重置", async () => {
    const productionAttempt = await login(
      "https://api.example.com/api/v1/auth/login",
      LOCAL_PASSWORD,
    );
    expect(productionAttempt.status).toBe(401);

    const DB = (env as any).DB as D1Database;
    const countAfterProductionAttempt = await DB.prepare(
      "SELECT COUNT(*) AS count FROM users",
    ).first<{ count: number }>();
    expect(countAfterProductionAttempt?.count).toBe(0);
