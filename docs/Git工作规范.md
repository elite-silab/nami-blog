...
- ✅ GitHub Actions CI 与 Cloudflare Git 集成部署流程已配置

开始贡献时，请参考 README.md 的快速开始指南搭建本地环境。本地空数据库可使用 `.env` 模板密码首次登录；生产管理员使用 Dashboard 一次性 Secret 或受控 seed 创建，绝不使用本地默认密码。
