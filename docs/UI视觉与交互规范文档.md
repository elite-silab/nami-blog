...
项目提供三套二次元风格主题，通过 `data-theme` 属性切换 CSS 变量。管理后台「站点设置 → 主题外观」设置全站默认，公开站点导航中的主题选择器保存访客本机偏好。
...
- 服务端：从 D1 `site_settings` 表的 `site_theme` 字段读取全站默认，通过 `data-theme` 属性注入 HTML
- 客户端：内联脚本校验并读取 `localStorage` 访客偏好，在首次绘制前覆盖默认值
- 后台切换：点击主题卡片即时预览，保存到 D1 并清理公开缓存
- 访客切换：前台即时更新 `data-theme`，具体主题保存在当前浏览器，“跟随站点”不保存覆盖
...
- 桌面端：导航右侧胶囊按钮 + 2 × 2 弹出面板
- 移动端：展开菜单内直接展示 2 × 2 选项，不叠加弹窗
- 视觉：主题色双点、名称、简短氛围和选中边框，不使用顶部彩色条
- 操作：支持键盘焦点、`Escape` 和点击外部关闭
